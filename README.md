...
**Literature-Assisted Dual-annotation and Evidence-based Reasoning**

A Python package for LLM-powered gene set dual annotation and literature validation, designed for computational biology workflows in cancer genomics and multi-omics research.
...
```
Gene set → [1] Dual Annotation (Direct LLM annotation + Enrichment Driven LLM Annotation)
...
| `proc_with` / `conf_with`                                | Annotation from the enrichment-driven annotation module |
| `proc_without` / `conf_without`                          | Annotation from the direct-LLM annotation module        |
| `analysis_with` / `analysis_without`                     | Detailed LLM analysis text for each annotation module   |
| `reasoning_with` / `reasoning_without`                   | Pathway reasoning for each annotation module            |
| `contributing_genes_with` / `contributing_genes_without` | Key genes associated with each annotation module        |

### Validation

| Field                                    | Description                                              |
| ---------------------------------------- | -------------------------------------------------------- |
| `final_process`                          | Final Literature-validated winning process name          |
| `final_conf`                             | Final confidence score (0–1)                             |
| `conf_with_after` / `conf_without_after` | Updated confidence scores after validation module        |
...
> _LADDER: Literature-Assisted Dual-annotation and Evidence-based Reasoning for gene set annotation_ (manuscript in preparation)
